...
- **Motion** — calm, declarative, no hype. Motion follows the site's print metaphor — flat at rest, inverted on hover, a press when held — and is quick, mechanical, and never decorative. The Signal Band brand itself sets no motion rules; what it does set and motion must respect is one stripe run per surface and no dark green on deep forest.
  ...
  **Approved patterns:**
  ...
  - Cross-page crossfade: `@view-transition { navigation: auto }`, 200ms. No element carries a `view-transition-name`: the header is in flow, not sticky, so a named header group animates from wherever the old page was scrolled and swoops in from above.
  - Signal stripe: the full-size `.stripes` band draws in once, scroll-driven (`animation-timeline: view()`), complete once fully visible. The slim and vertical bands are static. Keep `animation-timeline` out of the rule that sets the `animation` shorthand — the minifier folds it in and Chrome drops the declaration. Never put a scroll-driven element inside an `overflow: hidden` ancestor: that ancestor becomes the scroll container `view()` measures against, and the animation never progresses. Name the timeline on an unclipped ancestor instead (`view-timeline`). Verify by sampling the computed transform at several scroll offsets in a browser; an attached animation is not proof that it moves.
  - Content links thicken their underline to 3px on hover.
  - Mobile menu: `<button popovertarget="mobile-nav">` + `<nav id="mobile-nav" popover>`, no JS. The button is a printed text button labelled from `menuLabel` in `src/content/nav.ts`. The panel unrolls with a `clip-path` keyframe animation and closes with a `clip-path` transition (`display`/`overlay` `allow-discrete`); links stagger by `var(--dur-fast) / 3`, index capped at 4. Panel layout: top- and left-aligned list (never `justify-content: center` — it clips the first link once the list overflows), current page marked by an aqua leading rule, language chips pinned low, a static 1.25rem five-band stripe as the closing edge, and a 1px aqua hairline under the hero bar (never dark green: it is the one colour ruled out on deep forest, at 1.7:1). Do not use `@starting-style`: Siteimprove Alfa (the e2e a11y gate) cannot parse it and drops the rest of the stylesheet, failing sia-r73 site-wide. Without the Popover API the links render inline.
...
- The header switches at the 1199/1200px desktop breakpoint, not the 768px page layout: below it (phones, tablets and small laptops) the header is the 60px bar with the popover menu; from 1200px it is a 68px bar on every page type, hero and solid alike, with the wordmark overhanging the bar (absolute, `top: 14px`) and the nav padded clear of it. Do not move the switch lower: at 1000px "Ota yhteyttä" already wraps onto two lines. The e2e page objects treat anything under 1200px as mobile, so the two must move together. The front page hides the bar wordmark wherever its desktop hero shows the large one (≥769px).
