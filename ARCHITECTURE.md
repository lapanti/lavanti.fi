...
**Visible FAQ plate.** `src/components/Faq.astro` renders a locale's `faq` frontmatter as a gray plate. `PostLayout` and `NewsletterLayout` mount it automatically between the prose sheet and the other-writings plate; `PageLayout` mounts it below the page body only when the page passes `faqSection: true`, because a `PageLayout` body is composed by the page itself and the election pages already place `<Faq>` mid-body with their own headings. All three, and the `FAQPage` JSON-LD in `Head.astro`, gate on `hasFaqSection()` in `src/lib/faq.ts` (two entries or more — a FAQPage rich-result requirement), with the plate's trilingual labels in `faqSectionLabels` beside it. Marked-up FAQ content has to be visible on the page for the structured data to be eligible.
