...
describe('<NewsletterProvenance />', () => {
    ...
    it('renders the send date in the locale sentence with a machine-readable <time>', async () => {
        ...
        expect(result.querySelector('p')).toHaveTextContent('Lähetetty tilaajille 14.3.2026.')
        expect(result.querySelector('time')).toHaveAttribute('datetime', '2026-03-14')
        expect(getByText(result, '14.3.2026')).toBeDefined()
    })
    ...
    it('localises the sentence', async () => {
        ...
        expect(en.querySelector('p')).toHaveTextContent('Sent to subscribers on 14.3.2026.')
        expect(sv.querySelector('p')).toHaveTextContent('Skickat till prenumeranterna 2.11.2026.')
    })
})
